...
public class ValuePromptBuilder<TValue>(string prompt)
    ...
    private Func<TValue, IValidationResult>? _validator;
    ...
    public ILinePromptBuilder<TValue> AddValidation(Func<TValue, IValidationResult> validate) {
        var oldValidator = _validator;
        _validator = value => {
            var result = (ValidationResult<TValue>)(oldValidator is null ? Success(value) : oldValidator(value));
            result += (ValidationResult<TValue>)validate(value);
            return result;
        };
        return this;
    }
    ...
    private IValidationResult ValidateChoices(TValue value)
