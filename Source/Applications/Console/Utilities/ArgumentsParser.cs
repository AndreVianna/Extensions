...
public static class ArgumentsParser {
    public static async Task<IValidationResult> Parse(IHasChildren node, IReadOnlyList<string> arguments, CancellationToken ct) {
        var result = Results.ValidationResult.Success();
        ResetContext(node);
    ...
    private static async Task<(IValidationResult result, int index)> TrySetupChild(IHasChildren node, IReadOnlyList<string> arguments, int index, CancellationToken ct) {
    ...
    private static async Task<IValidationResult> ReadParameters(IHasChildren node, IReadOnlyList<string> arguments, CancellationToken ct) {
        ...
        foreach (var parameter in node.Parameters) {
            ...
            result = result.Add(arguments[index].StartsWith('-')
    ...
    private static IValidationResult EnsureAllRequiredParametersAreSet(IHasChildren node, IValidationResult result) {
