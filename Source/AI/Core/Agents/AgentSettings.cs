...
public class AgentSettings
    ...
    public Result Validate(IMap? context = null) {
        ...
        if (StopSequences.Count > 0 && StopSequences.Any(string.IsNullOrWhiteSpace))
            result += new Error("StopWaiting signals cannot be null, empty, or contain only whitespace.", nameof(StopSequences));
        if (MaximumNumberOfRetries > _maximumRetries)
            result += new Error($"The maximum number of retries is {_maximumRetries}. Found: {MaximumNumberOfRetries}", nameof(MaximumNumberOfRetries));
        if (Temperature is < 0 or > _maximumTemperature)
            result += new Error($"Content must be between {0} and {_maximumTemperature}. Found: {Temperature}", nameof(Temperature));
        if (TokenProbabilityCutOff is < 0 or > _maximumTokenProbabilityCutOff)
            result += new Error($"Content must be between {0} and {_maximumTokenProbabilityCutOff}. Found: {TokenProbabilityCutOff}", nameof(TokenProbabilityCutOff));
